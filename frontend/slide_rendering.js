/*
    ...
    У нас есть несколько типов элементов: текст text, изображение image, метрика metric, геометрическая фигура shape. "badge", не отличающийся особо ничем от остального текста, уходит. Уходят также и малоинтуитивные, по-моему, роли, каждый элемент определяется только теми параметрами, которые заданы в его полях.

    Некоторые группы полей могут применяться к нескольким типам объектов. Например, все поля, нужные для форматирования текста, могут также применяться к замещающему тексту изображения, тексту на графике метрики или к метрике-одиночному значению. Чтоб выделить их среди остальных, они вынесены в отдельный js-объект. В layout остаются только те поля, которые применимы ко всем элементам. Это 1) поля, отвечающие за его положение на холсте 2) поля, отвечающие за простые эффекты: прозрачность, поворот, цвет фона, рамка и ее цвет-толщина-скругление углов.

    Поля, которые применимы только к этому типу элементов, или которые нецелесообразно выделять в отдельную группу, так и остаются отдельными.

    Текст проще всего: у него только одно собственное поле, его содержимое.
    ...
    Немного про layout. Планировалось 2 способа расположения элемента на экране: absolute, когда положение элемента определяется только его координатами x y, и тогда он может накладываться на другие (и порядок этого наложения определяется полем z-index), и relative, когда они строго следуют друг за другом в том порядке, в котором перечислены, а расстояние между ними изменяется за счет полей margin, x и y не учитываются в таком случае. Горизонтальное выравнивание элемента относительно границ холста в режиме relative определяется полем align.

    Также есть идея убрать из layout width и height, потому что они вредят отображению текстового элемента, который может не влезть в отведенное ему поле. Едва ли нужен специально такой эффект. Может, для этого типа длине и ширине лучше рассчитываться автоматически? А остается эта пара полей для таких элементов, как изображение, метрика, фигура, где оно буквально задает размер графического элемента (для текста эту роль выполняют объем текста и размер шрифта)
    ...
    Как что должно задаваться? Сколько форм и обработчиков событий должно быть?

    text:
        content — появляющаяся при нажатии форма. Можно сделать скрытой, но держать на ней фокус, пока пользователь не перекликнет мышкой на другой предмет
        Все элементы объекта text - также формы.
...
var defaultLayoutSettingsObject = {
    
}

var defaultTextSettingsObject = {

}

var defaultFigureSettingsObject = {

}

var defaultChartSettingsObject = {
