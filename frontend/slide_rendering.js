...
function renderElement(element) {
    ...
    if (type === "chart" || type === "metric") {
        return renderMetricElement(element);
    }

    if (type === "badge") {
        return renderBadgeElement(element);
    }
...
function renderMetricElement(element) {
    ...
    if (
        chartType === "value" ||
        (!chart.datasets && element.value !== undefined)
    ) {
...
function applySlideBackground(canvas, background) {
    ...
    if (type === "image" && value) {
        ...
        canvas.style.backgroundImage =
            "url(" + JSON.stringify(toAbsoluteUrl(value)) + ")";
    ...
    if (type === "gradient") {
        ...
        const knownPreset = knownPresets.includes(preset)
            ? preset
            : "default";
...
function applyLayout(node, layout = {}, defaults = {}) {
    ...
    if (
        ...
        node.style.color = isSafeCssColor(layout.color)
            ? layout.color
            : defaults.color;
    }
    ...
    if (
        ...
        isSafeCssColor(layout.backgroundColor)
    ) {
    ...
    if (
        layout.borderColor &&
        isSafeCssColor(layout.borderColor)
    ) {
...
function applyImageClasses(node, layout, role) {
    ...
    if (["cover", "fill"].includes(fit)) {
        node.classList.add("fit-" + fit);
    }

    if (["rounded", "circle"].includes(shape)) {
    ...
    if (layout.shadow) {
        node.classList.add("shadow-soft");
    }
}
...
function applyRoleClass(node, role) {
    ...
    const roleClass = sanitizeClassName(
        String(role)
    );
...
// TODO: удалить после рефакторинга и перевести весь проект
// на новую функцию. Старое имя сохранено для совместимости.
function scaleCurrentCanvas(root) {
    ...
    const canvas = targetRoot.querySelector(
        ".slide-canvas"
    );
